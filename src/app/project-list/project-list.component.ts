...
export class ProjectListComponent implements OnInit {
  ...
  ngAfterViewInit() {
    this.renderer.listen('window', 'click', (e: Event) => {
      if (
        !this.typeButton.nativeElement.contains(e.target) &&
        !this.fromButton.nativeElement.contains(e.target) &&
        !this.byButton.nativeElement.contains(e.target)
      ) {
  ...
  selectDropdown(dt: DropdownType, selection: string) {
    ...
    this.router.navigate([], {
      ...
      queryParams: { type: this.type, from: this.from, by: this.sortedBy },
  ...
  filterDropdown() {
    ...
    if (this.type !== 'everything') {
      filtered = allProjects.filter((project) =>
        project.hashtags.has(this.type),
      );
    }
